...
def IsMoveLegal(board, from_square, to_square):
    ...
    if piece_to == '.' or piece_to.islower():
        occupantWhite = True
    if piece_to == '.' or piece_to.isupper():
        occupantBlack = True
    ...
    # Rook
        ...
        if x_to == x or y == y_to:
            if occupantBlack and isStraightClear(from_square, to_square, board):
                return True
    elif piece == "R": # white rook
        if x_to == x or y == y_to: #checks to see if its the same row or column
            if occupantWhite and isStraightClear(from_square, to_square, board): #checks if enemy or blank and pathing
                return True
    
    # Knight
    elif piece == "t": # black knight
        if occupantBlack and (x_abs == 2 and y_abs == 1) or (y_abs == 2 and x_abs == 1) :
            return True
    elif piece == "T": # white knight
        if occupantWhite and(x_abs == 2 and y_abs == 1) or (y_abs == 2 and x_abs == 1):
            return True
    # Bishop
    elif piece == "b": # black bishop
        if x == x_to:
            return False  
        if slope == 1  and occupantBlack and isDiagonalClear(from_square, to_square, board):
            return True

    elif piece == "B": # white bishop
        if x == x_to:
            return False
        if slope == 1 and occupantWhite and isDiagonalClear(from_square, to_square, board):
            return True
    # Queen
    ...
    # King
         ...
         if x_abs <= 1 and y_abs <= 1 and occupantBlack:
             return True
    elif piece == "K": # white king
        if x_abs <= 1 and y_abs <= 1 and occupantWhite:
             return True
    else:
       return False
    ...
    for frm_y in range(0, 8):
        for frm_x in range(0, 8):
            ...
            if turn == 'black' and board[frm_y][frm_x].islower():
                ...
                for to_y in range(0, 8):
                  for to_x in range(0, 8):
                    if IsMoveLegal(board, (frm_y, frm_x), (to_y, to_x)) and DoesMovePutPlayerInCheck(board, (frm_y, frm_x), (to_y, to_x), "black") is False:
                        to_list.append((to_y,to_x))
                if len(to_list) != 0:
                    legalMoves.update({(frm_y, frm_x):to_list})
    return legalMoves
...
def IsCheckmate(board, turn):
    ...
    # else
        # return True
    if IsInCheck(board, turn) and GetListOfLegalMoves(board, turn) is {}:
        return True
    else:
        return False
...
def DoesMovePutPlayerInCheck(board, from_square, to_square, turn):
    ...
    isCheck = IsInCheck(MovePiece(new_board, from_square, to_square), turn)
...
board[6][7] = '.'
temp = copy.deepcopy(board)
print(GetListOfLegalMoves(temp, turn))
DrawBoard(board)
to = (4,7)
from_ = (3,7)
print(DoesMovePutPlayerInCheck(board, from_, to, 'black'))
print(IsInCheck(board, turn))
